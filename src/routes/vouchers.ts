...
router.post("/validate", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    const [rows]: any = await pool.query("SELECT * FROM vouchers WHERE code = ? AND status = 'ACTIVE' LIMIT 1", [code]);
    const voucher = rows && rows[0];
    if (!voucher) return res.status(404).json({ valid: false, message: "Voucher không tồn tại hoặc không hoạt động" });
    ...
    if (voucher.applicable_courses) {
      let arr: string[] = [];
      try { arr = JSON.parse(voucher.applicable_courses); } catch (e) { arr = []; }
 ...
 */
router.post("/apply-to-invoice/:invoiceId", requireAuth, async (req: Request, res: Response) => {
  const { code } = req.body || {};
  ...
  try {
    ...
    // lock voucher row
    const [vRows]: any = await conn.query("SELECT * FROM vouchers WHERE code = ? FOR UPDATE", [code]);
    ...
    const [updatedRows]: any = await pool.query("SELECT i.*, (SELECT name FROM students WHERE id = i.student_id) as student_name FROM invoices i WHERE i.id = ?", [invoiceId]);
