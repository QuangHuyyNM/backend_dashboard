...
router.get("/overview", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    const monthQuery = (req.query.month as string) || new Date().toISOString().slice(0, 7);
    const [year, month] = monthQuery.split("-").map(Number);
    
    // ✅ FIX: Xử lý kết quả query một cách an toàn
    const [kpiRevenueRows]: any = await pool.query(
      "SELECT SUM(total_amount) as total FROM invoices WHERE YEAR(issued_date) = ? AND MONTH(issued_date) = ?",
      [year, month]
    );
    const kpiRevenue = kpiRevenueRows[0];

    const [kpiCollectedRows]: any = await pool.query(
      "SELECT SUM(amount) as total FROM payments WHERE YEAR(paid_at) = ? AND MONTH(paid_at) = ?",
      [year, month]
    );
    const kpiCollected = kpiCollectedRows[0];

    const [kpiExpensesRows]: any = await pool.query(
      "SELECT SUM(amount) as total FROM expenses WHERE YEAR(occurred_at) = ? AND MONTH(occurred_at) = ?",
      [year, month]
    );
    const kpiExpenses = kpiExpensesRows[0];

    const [kpiPayrollRows]: any = await pool.query(
      "SELECT SUM(total_pay) as total FROM payslips WHERE year = ? AND month = ?",
      [year, month]
    );
    const kpiPayroll = kpiPayrollRows[0];

    const [kpiReceivablesRows]: any = await pool.query(
      "SELECT SUM(total_amount - paid_amount) as total FROM invoices WHERE status NOT IN ('PAID', 'CANCELLED')"
    );
    ...
    const [topUnpaid]: any = await pool.query(
...
router.delete("/invoices/:id", requireAuth, requireRole(['CEO', 'FINANCE']), async (req: AuthenticatedRequest, res) => {
    try {
        const { id } = req.params;
        const [result]: any = await pool.query("DELETE FROM invoices WHERE id = ?", [id]);
...
router.get("/invoices/:id", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
        ...
        const [payments] = await pool.query("SELECT * FROM payments WHERE invoice_id = ? ORDER BY paid_at DESC", [id]);
...
// --- CÁC ROUTE KHÁC ---
router.get("/expenses", requireAuth, async (req: AuthenticatedRequest, res) => {
  const [rows] = await pool.query("SELECT * FROM expenses ORDER BY occurred_at DESC");
  res.json({ rows });
});

router.get("/payslips", requireAuth, async (req: AuthenticatedRequest, res) => {
  const [rows] = await pool.query(`
