...
async function runRemindersOnce() {
    try {
      ...
      `, [d]);
            for (const r of rows || []) {
                await db_1.default.query("INSERT INTO invoice_reminder_logs (invoice_id, reminder_type, channel, details) VALUES (?, ?, 'INAPP', ?)", [r.id, `DAYS_${d}`, JSON.stringify({ invoice_number: r.invoice_number })]);
                console.info(`[reminderWorker] queued DAYS_${d} reminder for invoice ${r.invoice_number}`);
                // TODO: call external email/sms send here
            }
        }
        // overdue
        const [overRows] = await db_1.default.query(`
      SELECT i.id, i.invoice_number, i.student_id, i.due_date, i.total_amount, i.paid_amount
      FROM invoices i
      WHERE i.status IN ('UNPAID','PARTIAL') AND i.due_date IS NOT NULL AND i.due_date < CURDATE()
    `);
        for (const r of overRows || []) {
            await db_1.default.query("INSERT INTO invoice_reminder_logs (invoice_id, reminder_type, channel, details) VALUES (?, 'OVERDUE', 'INAPP', ?)", [r.id, JSON.stringify({ invoice_number: r.invoice_number })]);
...
function startReminderWorker(intervalMs = 1000 * 60 * 60) {
    // run once at startup
