...
async function employeeExists(empId) {
    if (!empId)
        return false;
    const [rows] = await db_1.default.query("SELECT 1 FROM employees WHERE id = ? LIMIT 1", [empId]);
    return Array.isArray(rows) && rows.length > 0;
}
/* Helper: tồn tại class? */
async function classExists(classCode) {
    if (!classCode)
        return false;
    const [rows] = await db_1.default.query("SELECT 1 FROM classes WHERE class_code = ? LIMIT 1", [classCode]);
    return Array.isArray(rows) && rows.length > 0;
}
/**
 ...
 */
async function deduceFromSessionId(sessionId) {
    if (!sessionId)
        return null;
    ...
    const [rows] = await db_1.default.query("SELECT id FROM employees WHERE id LIKE ? LIMIT 1", [`%${lastToken}%`]);
    if (Array.isArray(rows) && rows.length > 0) {
        return { employee_id: rows[0].id, date, class_code };
    }
    return null;
}
/* GET list */
...
router.post("/", async (req, res) => {
    ...
    `;
        try {
            await db_1.default.query(sql, [session_id, finalEmployeeId, finalDate, finalClassCode || null, finalStatus]);
        }
        catch (err) {
        ...
        const [rows] = await db_1.default.query("SELECT * FROM staff_attendance WHERE session_id = ?", [session_id]);
        const saved = rows[0];
...
router.put("/:session_id", async (req, res) => {
    try {
        ...
        const [updateRes] = await db_1.default.query("UPDATE staff_attendance SET status = ?, updated_at = NOW() WHERE session_id = ?", [st, sessionId]);
        ...
        if (affected === 0) {
          ...
          ON DUPLICATE KEY UPDATE
            ...
            updated_at = NOW()
        `;
                await db_1.default.query(insertSql, [sessionId, ded.employee_id, ded.date, finalClass, st]);
            }
            catch (err) {
        ...
        const [rows] = await db_1.default.query("SELECT * FROM staff_attendance WHERE session_id = ?", [sessionId]);
...
router.delete("/:session_id", async (req, res) => {
    try {
        ...
        if (!sessionId)
            return res.status(400).json({ message: "Missing session id" });
        await db_1.default.query("DELETE FROM staff_attendance WHERE session_id = ?", [sessionId]);
        res.json({ ok: true });
    }
    catch (err) {
