...
function safeParseArrayField(val) {
    if (val == null)
        return [];
 ...
 */
function normalizeRolesInput(val) {
    if (val == null)
        return [];
    let arr = [];
    if (Array.isArray(val)) {
 ...
 */
function normalizePermissionsInput(val) {
    if (val == null)
        return [];
    let arr = [];
    if (Array.isArray(val))
...
router.get("/", async (_req, res) => {
    try {
        const [rows] = await db_1.default.query("SELECT * FROM employees WHERE role <> 'CEO' ORDER BY id");
...
router.get("/:id", async (req, res) => {
    try {
        const [rows] = await db_1.default.query("SELECT * FROM employees WHERE id = ?", [
            req.params.id,
        ]);
        const arr = rows;
        if (!arr.length)
        ...
        if ((arr[0].role || "").toUpperCase() === "CEO") {
            return res.status(403).json({
                message: "CEO is system user and cannot be accessed via employees endpoint.",
            });
        }
        const r = arr[0];
...
router.post("/", async (req, res) => {
    try {
        ...
        if (!body.id) {
            const [rows] = await db_1.default.query("SELECT MAX(CAST(SUBSTRING(id, 4) AS UNSIGNED)) as max_num FROM employees WHERE id LIKE 'RTE%'");
        ...
        if (String(body.role).toUpperCase() === "CEO") {
            return res.status(403).json({
                message: "Không thể tạo nhân sự có vai trò CEO qua HR (chỉ DB/systemUsers).",
            });
        }
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const [targetRows] = await db_1.default.query("SELECT role FROM employees WHERE id = ?", [id]);
        const arr = targetRows;
        if (!arr.length)
            return res.status(404).json({ message: "Not found" });
        if ((arr[0].role || "").toUpperCase() === "CEO") {
            return res.status(403).json({
        ...
        let permissionsPayload = undefined;
...
router.delete("/:id", async (req, res) => {
    try {
        const [rows] = await db_1.default.query("SELECT role FROM employees WHERE id = ?", [
            req.params.id,
        ]);
        const arr = rows;
        if (!arr.length)
        ...
        await db_1.default.query("DELETE FROM employees WHERE id = ?", [req.params.id]);
        res.json({ ok: true });
    }
    catch (err) {
