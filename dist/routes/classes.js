...
const db_1 = __importDefault(require("../db")); // mong là bạn export mysql2/promise pool ở đây
 ...
 */
async function generateUniqueClassCode(courseId) {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        // check exists
        const [rows] = await db_1.default.query("SELECT 1 FROM classes WHERE class_code = ? LIMIT 1", [candidate]);
        if ((rows || []).length === 0)
            return candidate;
...
router.post("/", async (req, res) => {
    const { classCode, courseId, teacherId = null, taId = null, schedule, maxStudents = 15, status = "Sắp khai giảng", studentIds = null, // if you have class_students table you should insert later
    notes = null, } = req.body || {};
    ...
    try {
        ...
        if (codeToUse) {
            // ensure not exists
            const [exists] = await db_1.default.query("SELECT 1 FROM classes WHERE class_code = ? LIMIT 1", [codeToUse]);
            if ((exists || []).length > 0) {
        ...
        if (Array.isArray(studentIds) && studentIds.length > 0) {
            ...
            try {
                await db_1.default.query("INSERT IGNORE INTO class_students (class_code, student_id) VALUES ?", [insertRows]);
            }
            catch (e) {
        ...
        const [createdRows] = await db_1.default.query(`SELECT c.class_code, c.course_id, co.name AS course_name, c.teacher_id, t.name AS teacher_name,
              c.ta_id, ta.name AS ta_name, c.schedule, c.max_students, c.status, c.created_at, c.updated_at
       FROM classes c
       LEFT JOIN courses co ON co.id = c.course_id
       LEFT JOIN employees t ON t.id = c.teacher_id
       LEFT JOIN employees ta ON ta.id = c.ta_id
       WHERE c.class_code = ? LIMIT 1`, [codeToUse]);
...
router.put("/:id", async (req, res) => {
    ...
    const { courseId, teacherId, taId, schedule, maxStudents, status } = req.body || {};
    ...
    try {
        // check exists
        const [exists] = await db_1.default.query("SELECT 1 FROM classes WHERE class_code = ? LIMIT 1", [id]);
        if (exists.length === 0) {
            return res.status(404).json({ message: "Class not found" });
        }
        const sql = `UPDATE classes SET ${updates.join(", ")}, updated_at = CURRENT_TIMESTAMP WHERE class_code = ?`;
        params.push(id);
        await db_1.default.query(sql, params);
        // return updated record (with joins)
        const [rows] = await db_1.default.query(`SELECT c.class_code, c.course_id, co.name AS course_name, c.teacher_id, t.name AS teacher_name,
              c.ta_id, ta.name AS ta_name, c.schedule, c.max_students, c.status, c.created_at, c.updated_at
       FROM classes c
       LEFT JOIN courses co ON co.id = c.course_id
       LEFT JOIN employees t ON t.id = c.teacher_id
       LEFT JOIN employees ta ON ta.id = c.ta_id
       WHERE c.class_code = ? LIMIT 1`, [id]);
        res.json((rows || [])[0]);
    }
    catch (err) {
...
router.delete("/:id", async (req, res) => {
    ...
    try {
        ...
        try {
            await db_1.default.query("DELETE FROM class_students WHERE class_code = ?", [id]);
        }
        catch (e) {
            // ignore if table not exists or other minor errors
        }
        await db_1.default.query("DELETE FROM classes WHERE class_code = ?", [id]);
