...
async function tableExists(tableName) {
    const [rows] = await db_1.default.query("SELECT COUNT(*) as cnt FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = ?", [tableName]);
    return (rows && rows[0] && Number(rows[0].cnt) > 0) || false;
}
// Helper: check if column exists in table
async function columnExists(tableName, columnName) {
    const [rows] = await db_1.default.query("SELECT COUNT(*) as cnt FROM information_schema.columns WHERE table_schema = DATABASE() AND table_name = ? AND column_name = ?", [tableName, columnName]);
    return (rows && rows[0] && Number(rows[0].cnt) > 0) || false;
}
// Utility: normalize a payslip raw db row into JS object with numbers
function normalizePayslipRow(r) {
    if (!r)
        return null;
...
async function ensureTablesExist() {
    try {
        await db_1.default.query(`CREATE TABLE IF NOT EXISTS payslips (
      ...
      updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
        ...
        await db_1.default.query(`CREATE TABLE IF NOT EXISTS payslip_payments (
...
ensureTablesExist().catch((e) => {
    console.warn("[payslips] ensureTablesExist failed (non-fatal):", e?.message || e);
});
...
async function sendPaymentReceiptEmail(toEmail, payslip, payment) {
    try {
    ...
    `;
        ...
        console.error("[payslips] sendPaymentReceiptEmail failed (non-fatal):", e?.message || e);
    }
}
/**
...
async function computeAttendanceMetrics(employeeId, month, year) {
    ...
    const [rowsDuration] = await db_1.default.query(`SELECT 
       COUNT(sa.session_id) AS sessions_count
     FROM staff_attendance sa
     WHERE sa.employee_id = ? AND sa.status = 'PRESENT' AND MONTH(sa.date)=? AND YEAR(sa.date)=?`, [employeeId, month, year]);
    const sessions = (rowsDuration &&
        rowsDuration[0] &&
    ...
    if (hasTeachingLogs) {
        ...
        const [durRows] = await db_1.default.query(`SELECT SUM(
         IF(
       ...
       FROM teaching_logs
       WHERE teacher_id = ? AND MONTH(date)=? AND YEAR(date)=?`, [employeeId, month, year]);
 ...
 */
router.post("/run", auth_middleware_1.requireAuth, (0, auth_middleware_1.requirePermission)("RUN_PAYROLL"), async (req, res) => {
    ...
    try {
        ...
        const [emps] = await conn.query(`SELECT id, name, role, hourly_rate FROM employees WHERE (hourly_rate IS NOT NULL AND hourly_rate > 0) OR role IN ('TEACHER','TA')`);
        ...
        const [existing] = await conn.query(`SELECT COUNT(*) as cnt FROM payslips WHERE month=? AND year=?`, [month, year]);
        const existingCount = (existing && existing[0] && Number(existing[0].cnt)) || 0;
        ...
        if (existingCount > 0 && force) {
            ...
            await conn.query(`DELETE p FROM payslip_payments p
        ...
        for (const e of emps) {
            ...
            const { sessions, hours } = await computeAttendanceMetrics(empId, month, year);
            ...
            await conn.query(`INSERT INTO payslips (id, employee_id, employee_name, employee_role, month, year, teaching_sessions, teaching_hours, hourly_rate, total_pay, paid_amount, status, created_at)
...
router.get("/", auth_middleware_1.requireAuth, async (req, res) => {
    try {
        ...
        // count
        const [countRows] = await db_1.default.query(`SELECT COUNT(*) as cnt FROM payslips ${where}`, params);
        const total = (countRows && countRows[0] && Number(countRows[0].cnt)) || 0;
        const [rows] = await db_1.default.query(`SELECT * FROM payslips ${where} ORDER BY COALESCE(created_at, updated_at, '1970-01-01') DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
...
router.get("/:id", auth_middleware_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db_1.default.query(`SELECT * FROM payslips WHERE id = ? LIMIT 1`, [id]);
        ...
        const payOrder = (await columnExists("payslip_payments", "created_at"))
            ...
            : (await columnExists("payslip_payments", "paid_at"))
                ? "paid_at DESC"
                : "id DESC";
        const [payments] = await db_1.default.query(`SELECT * FROM payslip_payments WHERE payslip_id = ? ORDER BY ${payOrder}`, [payslip.id]);
 ...
 */
router.post("/:id/pay", auth_middleware_1.requireAuth, (0, auth_middleware_1.requirePermission)("RUN_PAYROLL"), async (req, res) => {
    ...
    try {
        ...
        const [rows] = await conn.query(`SELECT * FROM payslips WHERE id = ? OR payslip_id = ? LIMIT 1`, [payslipIdParam, payslipIdParam]);
        const raw = (rows && rows[0]) || null;
        if (!raw) {
            await conn.rollback();
            conn.release();
        ...
        const [idColRows] = await conn.query(`SELECT EXTRA 
         FROM information_schema.columns 
         WHERE table_schema = DATABASE() 
           AND table_name = 'payslip_payments' 
           AND column_name = 'id'`);
        const hasAutoIncId = idColRows &&
            idColRows[0] &&
        ...
        const [tbl] = await conn.query(`SELECT COUNT(*) as cnt FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'payslip_payments'`);
        if (!(tbl && tbl[0] && Number(tbl[0].cnt) > 0)) {
            await conn.query(`CREATE TABLE IF NOT EXISTS payslip_payments (
             id BIGINT AUTO_INCREMENT PRIMARY KEY,
        ...
        if (hasAutoIncId) {
            await conn.query(`INSERT INTO payslip_payments 
        ...
        const [sumRows] = await conn.query(`SELECT SUM(amount) as tot FROM payslip_payments WHERE payslip_id = ?`, [payslip.id]);
        const totalPaid = Number(sumRows && sumRows[0] && sumRows[0].tot) || 0;
        // determine status
        const totalPayVal = Number(payslip.total_pay || 0);
        const newStatus = totalPayVal > 0 && totalPaid >= totalPayVal
            ? "Đã thanh toán"
            : "Chưa thanh toán";
        await conn.query(`UPDATE payslips SET paid_amount = ?, status = ?, updated_at = NOW() WHERE id = ?`, [totalPaid, newStatus, payslip.id]);
        // fetch updated payslip
        const [uRows] = await conn.query(`SELECT * FROM payslips WHERE id = ? LIMIT 1`, [payslip.id]);
        ...
        const payOrder = (await columnExists("payslip_payments", "created_at"))
            ...
            : (await columnExists("payslip_payments", "paid_at"))
                ? "paid_at DESC"
                : "id DESC";
        const [paymentsRows] = await conn.query(`SELECT * FROM payslip_payments WHERE payslip_id = ? ORDER BY ${payOrder}`, [payslip.id]);
        // send receipt email if employee email is available (non-fatal)
        try {
            let empEmail = null;
            if (updatedPayslip && updatedPayslip.employee_id) {
                const [eR] = await conn.query(`SELECT email FROM employees WHERE id = ? LIMIT 1`, [updatedPayslip.employee_id]);
                if (eR && eR[0] && eR[0].email)
                    empEmail = eR[0].email;
            }
            if (empEmail) {
                const lastPayment = paymentsRows && paymentsRows[0]
                    ? {
                        ...paymentsRows[0],
 ...
 */
router.post("/:id/recalculate", auth_middleware_1.requireAuth, (0, auth_middleware_1.requirePermission)("RUN_PAYROLL"), async (req, res) => {
    const idParam = req.params.id;
    const conn = await db_1.default.getConnection();
    try {
        await conn.beginTransaction();
        const [rows] = await conn.query(`SELECT * FROM payslips WHERE id=? LIMIT 1`, [idParam]);
        const raw = (rows && rows[0]) || null;
        if (!raw) {
            await conn.rollback();
            conn.release();
        ...
        const { sessions, hours } = await computeAttendanceMetrics(payslip.employee_id, payslip.month, payslip.year);
        const hourly = Number(payslip.hourly_rate || 0);
        const totalPay = Number(hours * hourly || 0);
        await conn.query(`UPDATE payslips SET teaching_sessions=?, teaching_hours=?, total_pay=?, updated_at = NOW() WHERE id = ?`, [sessions, hours, totalPay, payslip.id]);
        // fetch updated
        const [uRows] = await conn.query(`SELECT * FROM payslips WHERE id = ? LIMIT 1`, [payslip.id]);
 ...
 */
router.get("/:id/pdf", auth_middleware_1.requireAuth, async (req, res) => {
    const idParam = req.params.id;
    try {
        const [rows] = await db_1.default.query(`SELECT * FROM payslips WHERE id=? LIMIT 1`, [idParam]);
        const raw = (rows && rows[0]) || null;
        if (!raw)
            return res.status(404).json({ message: "Payslip không tìm thấy" });
        const payslip = normalizePayslipRow(raw);
        const [payslipPayments] = await db_1.default.query(`SELECT * FROM payslip_payments WHERE payslip_id = ? ORDER BY paid_at DESC`, [payslip.id]);
        // generate pdf
        const doc = new pdfkit_1.default({ size: "A4", margin: 40 });
        const writableBuffer = new stream_buffers_1.default.WritableStreamBuffer();
        ...
        if (payslipPayments && payslipPayments.length) {
            ...
            for (const p of payslipPayments) {
                doc.text(`- ${p.method || "—"} | ${Number(p.amount || 0).toFixed(2)} | ${p.reference || ""} | ${p.note || ""}`);
            }
        }
        doc.end();
        ...
        writableBuffer.on("finish", () => {
            ...
            res.setHeader("Content-Disposition", `attachment; filename="payslip-${payslip.employee_id || payslip.id}.pdf"`);
            res.send(buf);
        });
    }
    catch (err) {
 ...
 */
router.delete("/:id", auth_middleware_1.requireAuth, (0, auth_middleware_1.requirePermission)("RUN_PAYROLL"), async (req, res) => {
    const idParam = req.params.id;
    const conn = await db_1.default.getConnection();
    try {
        await conn.beginTransaction();
        const [rows] = await conn.query(`SELECT id FROM payslips WHERE id=? LIMIT 1`, [idParam]);
        const raw = (rows && rows[0]) || null;
        if (!raw) {
            await conn.rollback();
            conn.release();
        ...
        const [result] = await conn.query(`DELETE FROM payslips WHERE id = ?`, [payslipId]);
        await conn.commit();
        conn.release();
